...
{{
    ...
    async def _select_area_http(self, area_url: str) -> str | None:
        """用 HTTP GET 抓 area 頁 HTML，依 .zone 區塊精確綁定價格並選擇區域"""
        if not self._http:
            return None
        try:
            ...
            if not m:
                m = _re.search(r'areaUrlList\s*=\s*(\{[^}]+\})', html)

            if m:
                try:
                    url_map = _json.loads(m.group(1).replace("'", '"'))
                    if url_map:
                        # 完整相容 area_keywords (List) 與 area_keyword (Str)
                        raw_kw = getattr(self.event, "area_keywords", None) or getattr(self.event, "area_keyword", None) or []
                        ...
                        skip_re = _re.compile(r"身心障礙|身障|輪椅|wheelchair|殘障", _re.IGNORECASE)
                        max_price = getattr(self.event, "max_price", None)

                        zones = _re.findall(
                            r'(<div[^>]*class=["\'][^"\']*\bzone\b[^"\']*["\'][^>]*>.*?)(?=<div[^>]*class=["\'][^"\']*\bzone\b|$)',
                            html, _re.DOTALL | _re.IGNORECASE
                        )

                        candidates = []
                        for zone_html in zones:
                            zone_text = _re.sub(r"<[^>]+>", " ", zone_html)
                            zone_text = _re.sub(r"\s+", " ", _html.unescape(zone_text)).strip()
                            
                            # 只匹配帶有 $ 號的真實金額（例：$5380），排除區域號碼 220 誤判
                            prices = [int(p.replace(',', '')) for p in _re.findall(r'\$\s*([\d,]+)', zone_text)]
                            zone_min_price = min(prices) if prices else 999999

                            for aid, ticket_path in url_map.items():
                                escaped_id = _re.escape(str(aid))
                                if _re.search(rf'id=["\']{escaped_id}["\']|href=["\'][^"\']*{escaped_id}', zone_html):
                                    full_url = ticket_path if ticket_path.startswith("http") else f"https://tixcraft.com{ticket_path}"
                                    if full_url in self._failed_areas:
                                        continue

                                    a_match = _re.search(rf'<(?:a|button)[^>]*id=["\']{escaped_id}["\'][^>]*>(.*?)</(?:a|button)>', zone_html, _re.IGNORECASE | _re.DOTALL)
                                    btn_text = _re.sub(r"<[^>]+>", " ", a_match.group(1)) if a_match else ""
                                    full_area_name = f"{zone_text} {btn_text}".strip()

                                    if skip_re.search(full_area_name):
                                        continue
                                    ...
                                    candidates.append({
                                        ...
                                        "name": full_area_name,
                                        "url": full_url,
                                        "price": zone_min_price
                                    })

                        logger.info(
                            "HTTP 可買區域：%s",
                            "、".join(item["name"][:30] for item in candidates) or "無",
                        )

                        # 篩選出符合預算上限的區域
                        valid_price_candidates = [
                            item for item in candidates
                            if not max_price or item["price"] <= max_price
                        ]

                        # 【第一優先】關鍵字比對（只要符合陣列中任一關鍵字）
                        if kw_list and valid_price_candidates:
                            for item in valid_price_candidates:
                                if any(kw in item["name"] for kw in kw_list):
                                    logger.info("HTTP 成功匹配指定區域與價格：%s (價格:%s <= %s) → %s", item["name"][:40], item["price"], max_price, item["url"])
                                    return item["url"]

                            logger.warning("HTTP 指定區域 %r 在預算內已售完，啟動備用 (Fallback) 流程", kw_list)

                        # 【第二優先：Fallback】預算內第一個可用區
                        if valid_price_candidates:
                            selected = valid_price_candidates[0]
                            logger.info("HTTP 自動選取預算內最優可用區域 (Fallback)：%s (價格:%s <= %s) → %s", selected["name"][:40], selected["price"], max_price, selected["url"])
                            return selected["url"]

                        logger.warning("HTTP 沒有符合條件與價格限制 (<=%s 元) 的區域", max_price)
                        return None
                except Exception as exc:
                    logger.debug("解析 areaUrlList 失敗: %s", exc)
                    pass
            return None
        except Exception as e:
    ...
    async def _select_game(self) -> None:
        ...
        # HTTP 被 401 拒絕後，瀏覽器頁面可能仍在載入。
        # 最多等待約 4.5 秒，避免 rows.length === 0 被誤判為售完。
        for _ in range(30):
            game_info = await self.page.evaluate(f"""
                (() => {{
                    const rows =
                        document.querySelectorAll('{SEL_GAME_ROWS}');
            ...
            if game_info and game_info.get("total", 0) > 0:
                break

            await self.page.sleep(0.15)

        if not game_info or game_info.get("total", 0) == 0:
            delay = random.uniform(1.0, 2.0)

            logger.warning(
                "場次列表尚未載入或選擇器未命中，"
                "不是已售完；readyState=%s，%.1f 秒後重試...",
                (game_info or {}).get("ready_state", "unknown"),
                delay,
            )
    ...
    async def _select_area(self) -> None:
        """按照 area_keyword 與 max_price 選擇可購買區域。"""
        import json as _json
        ...
        keywords_json = _json.dumps(kw_list, ensure_ascii=False)
        max_price = getattr(self.event, "max_price", None) or 999999

        scan_result = None

        for _ in range(20):
            scan_result = await self.page.evaluate(rf"""
                (() => {{
                    ...
                    for (const zone of zones) {{
                        ...
                        if (skipRe.test(zoneText)) continue;

                        const priceMatches = zoneText.match(/\$\s*([\d,]+)/g);
                        let minPrice = 999999;
                        if (priceMatches) {{
                            const nums = priceMatches.map(p => parseInt(p.replace('$', '').replace(',', '')));
                        ...
                        for (const link of links) {{
                            ...
                            const fullText = (zoneText + ' ' + linkText).replace(/\s+/g, ' ');
                    ...
                    if (candidates.length === 0) {{
                        return {{ ready: true, reason: 'sold_out', available: 0, candidates: [] }};
                    }}

                    let target = null;
                    if (keywords && keywords.length > 0) {{
                        target = candidates.find(item => keywords.some(kw => item.text.includes(kw))) || null;
                    }}
            ...
            if scan_result and scan_result.get("ready"):
                break

            await self.page.sleep(0.15)

        if not scan_result or not scan_result.get("ready"):
            logger.warning("區域頁尚未載入完成，返回場次頁重試")
            await self._go_back_to_game()
            return

        candidates = scan_result.get("candidates", [])
        logger.info("瀏覽器可買區域：%s", "、".join(candidates) if candidates else "無")

        target = scan_result.get("target")

        if target:
            logger.info("選擇區域：%s → %s", target.get("text", ""), target.get("url", ""))
            await self.page.goto(target["url"])
            if not self.config.browser.turbo_mode:
                await self.page.sleep(0.3)
            return

        logger.warning("areaUrlList 中沒有任何可買區域")
        await self._go_back_to_game()
            ...
            try:
                ...
                scan_result = await self.page.evaluate(f"""
                    (() => {{
                        ...
                        let fallbackId = null, fallbackText = '', fallbackUrl = '';
                        ...
                        for (const zone of zones) {{
                            ...
                            for (const link of links) {{
                                ...
                                // 先記錄第一個可買區，作為最後 fallback
                                if (!fallbackId) {{
                                    fallbackId = link.id;
                                    fallbackText = linkText.substring(0, 60);
                                    fallbackUrl = linkUrl;
                                }}
                                ...
                                if (
                                    ...
                                    (
                                        ...
                                        zoneText.includes(keyword)
                                    )
                                ) {{
                                    targetId = link.id;
                                    targetText = linkText.substring(0, 60);
                                    targetUrl = linkUrl;
                                }}
                            }}
                        }}
